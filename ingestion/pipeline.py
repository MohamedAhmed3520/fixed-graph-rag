    ...
    def ingest_path(
        ...
        try:
            ...
            result = IngestionResult(
                document_id=(
                    f"doc-{hashlib.sha256(path.read_bytes()).hexdigest()[:24]}"
                ),
                filename=path.name,
                errors=[str(exc)],
                duration_seconds=(
            ...
            if (
                ...
                and "No text could be extracted" in str(exc)
            ):
                result.warnings.append(
                    "This PDF has no extractable text layer. "
                    "OCR is required for scanned/image-only PDFs."
                )

            logger.warning(
                "Document loading failed for %s: %s",
                path.name,
                exc,
            )
    ...
    def ingest_documents(
        ...
        try:
            ...
            if self.client and not allow_duplicate:
                ...
                if duplicate_rows:
                    ...
                    result.warnings.append(
                        ...
                        f"{duplicate_rows[0]['document_id']}"
                    )
            ...
            document = DocumentRecord(
                document_id,
                first_metadata["filename"],
                first_metadata["source"],
                first_metadata["file_type"],
                digest,
                text,
            )
            ...
            result.embeddings_created = len(
                vectors
            )

            result.entities_created = len(
                {
                    entity.entity_id
            ...
            if self.client:
                ...
                if not result.validation["valid"]:
                    ...
                    result.errors.append(
                        "Post-ingestion validation failed: "
                        f"expected 1 document and "
                        f"{len(chunks)} chunks, "
                        f"found "
                        f"{result.validation['documents']} "
                        "document(s) and "
                        f"{result.validation['chunks']} "
                        "chunk(s)."
                    )
